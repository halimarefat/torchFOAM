...
if __name__ == "__main__":
    ...
    time_R103, probe_data_R103 = probeFileReader(file_path_R103)
    time_R503, probe_data_R503 = probeFileReader(file_path_R503)
    time_R104, probe_data_R104 = probeFileReader(file_path_R104)
    ...
    if flag == 'partial':
        ...
        plt.legend()
                ...
                # Save the plot
        save_path = f'./Compare_energy_spectrum_probe_{probe_index}_U{velDir(dir_index)}.png'  # Update with your desired save path
        plt.savefig(save_path)
        plt.close()
        
    elif flag == 'full':
        file_path = file_path_R103
        probes = np.array(probe_data_R103)
        ...
        for probe_index in range(probe_count):
            for dir_index in range(3):
                ...
                # Save the plot
                save_path = f'./NN_Plots/{file_path.split("/")[-5]}_energy_spectrum_probe_{probe_index}_U{velDir(dir_index)}.png'  # Update with your desired save path
                plt.savefig(save_path)
                plt.close()
